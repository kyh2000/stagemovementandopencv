...
//========== 모션 구동중 및 구동후에 상태 확인하는 함수==============================================================
    ...
    // 지정 축의 절대치 엔코더 원점 Offset 위치를 설정한다.
...
//========= 오버라이드 함수 =========================================================================================
    ...
    //                : OVERRIDE_POS_START(0) 지정한 위치에서 지정한 속도로 오버라이드 시작함  
    //                : OVERRIDE_POS_END(1) 지정한 위치에서 지정한 속도가 되도록 미리 오버라이드 시작함
    //                : OVERRIDE_POS_AUTO(2) 지정한 위치에서 가속 구간에서는 지정한 속도로, 감속 구간에서는 지정한 속도가 되도록 미리 오버라이드를 시작함.
...
//-------------------------------------------------------------------------------------------------------------------
    ...
    // EtherCAT은 PlusTorqueLimit 값만으로 양쪽 다 같은 TorqueLimit을 건다.
    ...
    // 지정된 축의 Multi TorqueLimit을 설정(Enable or Disable) 한다.
    DWORD    __stdcall AxmMotMultiTorqueLimitEnable(long lAxisNo, DWORD dwEnable);

    // 지정된 축의 Multi TorqueLimit이 Enable 되어있는지 읽어 온다. 
    ...
    DWORD    __stdcall AxmCompensationOneDimSet(long lTableNo, long lSourceAxis, long lTargetAxis, long lSize, double* dpMotorPosition, double* dpLoadPosition);
    ...
    // 실시간 Conti Queue 모션을 설정한다.(Conti Queue = 128)
    // AxmContiQBegin : ContiQueue 모션 설정
    // lStartSegmCount : 입력되는 모션 세그먼트 개수가 lStartSegmCount일 때 모션이 시작된다.(1 ~ 127)
    // AxmContiQGetStatus()함수를 이용하여 plFreeSegmCount > 1(최소) 일때 다음 모션명령을 입력한다.(직선보간 또는 원/원호보간)
    // lStopMode = 0(Reserved)
    // AxmContiQEnd : ContiQueue 모션 해제
    DWORD    __stdcall AxmContiQBegin(long lCoordNo, long lStartSegmCount);
    DWORD    __stdcall AxmContiQEnd(long lCoordNo, long lStopMode);

    // Master 좌표계의 모션이 시작될 때 Slave 좌표계 모션이 시작된다.
    // lSlaveCoordSize : 최대 7
    // AxmContiQBegin 함수와 AxmContiQEnd 함수 사이에서 사용하여야 된다.
    DWORD    __stdcall AxmContiQSetSync(long lMasterCoordNo, long lSlaveCoordSize, long* plSlaveCoordNo);
    DWORD    __stdcall AxmContiQResetSync(long lMasterCoordNo);

    // 모션 세그먼트의 종료 지점에서 정지한다.
    // lRelSegmNo : 구동중인 모션 세그먼트를 기준으로 상대 모션세그먼트의 번호를 입력한다.(0 ~ 127) 
    // 0=현재 세그먼트의 종점에 감속정지 한다. 1=다음 세그먼트의 종점에 감속정지 한다. 
    // 감속도는 이동거리에 따라 자동 계산된다.
    DWORD    __stdcall AxmContiQMoveSegmStop(long lCoordNo, long lRelSegmNo);

    // 연속적인 모션 세그먼트 사이의 연결상태를 지정한다.
    // lMode : 연결상태 지정 
    //  '0' = 사용자 지정속도 구동
    //  '1' = 모션 세그먼트가 연결되는 부분에서 자동 가감속(AxmContiQSetConnectionAngle(), AxmContiQSetConnectionVelRate() 함수사용)
    //  '2' = 모션 세그먼트가 연결되는 부분에서 자동 원호 삽입(AxmContiQSetConnectionAngle(), AxmContiQSetConnectionFilletDistance() 함수사용)
    // AxmContiQBegin 함수와 AxmContiQEnd 함수 사이에서 사용하여야 된다.
    DWORD    __stdcall AxmContiQSetConnectionMode(long lCoordNo, long lMode);

    // ContiQSetConnectionMode 함수에서 lMode가 '1'이나 '2'로 지정되었을 때 유효하며
    // dAngle : 자동 가감속모드나 자동 원호삽입 모드에서 기준이 되는 각도로 0 < dAngle < 180  이다.
    // AxmContiQBegin 함수와 AxmContiQEnd 함수 사이에서 사용하여야 된다.
    DWORD    __stdcall AxmContiQSetConnectionAngle(long lCoordNo, double dAngle);

    // ContiQSetConnectionAngle 함수에서 설정된 각도에서의 속도 비율이다.(EndVel 기준)
    // dVelRate : 0 <= dVelRate <= 100
    // AxmContiQBegin 함수와 AxmContiQEnd 함수 사이에서 사용하여야 된다.
    DWORD    __stdcall AxmContiQSetConnectionVelRate(long lCoordNo, double dVelRate);

    // 자동 원호삽입 모드에서 모션 세그먼트의 종점과 원호의 접선사이의 거리(mm) 
    // dFilletDistance : 0 < dFilletDistance < 세그먼트 길이
    // AxmContiQBegin 함수와 AxmContiQEnd 함수 사이에서 사용하여야 된다.
    DWORD    __stdcall AxmContiQSetConnectionFilletDistance(long lCoordNo, double dFilletDistance);

    // Conti Queue모션은 128개의 모션 Queue를 가지고 있다.
    // plFreeSegmCount : 입력 가능한 모션 Queue 개수
    // plCurrentSegmNo : 현재 진행중인 Queue 번호
    // 최소 1개이상의 FreeSegmCount가 필요하다(모션 정지를 위한 1개 이상의 Free Queue 필요)
    DWORD    __stdcall AxmContiQGetStatus(long lCoordNo, long* plFreeSegmCount, long* plCurrentSegmNo);

    // Reserved
    //DWORD    __stdcall AxmContiQSetOutputBitPeriod(long lCoordNo, long lSize, long lModuleNo, DWORD dwModuleType, long* plBit, double* pdDistTime, double* pdWidth, long* plDistTimeMode);
    //DWORD    __stdcall AxmContiQResetOutputBitPeriod(long lCoordNo, long lModuleNo, DWORD dwModuleType, long lBit);

    // 특정 모션 세그먼트에서 I/O출력
    // lSize :  1 ~ 8
    // lModuleNo : dwModuleType=0 일때 는 축번호
    // dwModuleType : 0=Motion I/O Output, 1=Digital I/O Output
    // lRelSegmNo : 1 ~ 127 
    // plDistTimeMode : 0=거리(mm), 1=시간(msec), 2=거리비율(%), 모션 세그먼트 종점을 기준으로 한다.
    // AxmContiQBegin 함수와 AxmContiQEnd 함수 사이에서 사용하여야 된다.
    // Reserved
    //DWORD    __stdcall AxmContiQSetSegmOutputBit(long lCoordNo, long lSize, DWORD dwModuleType, long* plModuleNo, long* plBit, long* plOffOn, double* pdDistTime, long* plDistTimeMode);

    // Reserved
    //DWORD    __stdcall AxmContiQSetSegmOutputBitPeriod(long lCoordNo, long lSize, DWORD dwModuleType, long* plModuleNo, long* plBit, double* pdDistTime, double* pdStartOutput, double* pdEndOutput, double* pdWidth, long* plDistTimeMode);
    //DWORD    __stdcall AxmContiQResetSegmOutputBitPeriod(long lCoordNo, DWORD dwModuleType, long lModuleNo, long lBit);

    // 지정된 좌표계에 연속 보간 구동을 위해 저장된 내부 Queue를 모두 삭제하는 함수이다.
    DWORD    __stdcall AxmContiQWriteClear(long lCoordNo);
