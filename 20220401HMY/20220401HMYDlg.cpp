...
void CMy20220401HMYDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	else if(Set_Cursor == 2)
	{
		...
		else if (btn_rec)
		{
			...
			m_sliY.SetPos(nSliderEndPosY - rec_points[rec_click_count][1]);  //////  이 부분 역시 y좌표계를 상하 반전하여 적용
		...
		m_sliY.SetPos(nSliderEndPosY - pen_points[pen_click_count][1]);  //////  이 부분 역시 y좌표계를 상하 반전하여 적용

		pen_click_count++;

		if (pen_click_count == PEN_MAX)
		{
			AfxMessageBox(_T("작동이 시작됩니다. 설정을 다시 한번 확인하세요"));
			Invalidate();
			UpdateWindow();
...
UINT CMy20220401HMYDlg::VisionThreadStart(LPVOID pParam)
{
	...
	if (cap.isOpened())
	{
		while (line_thread->btn_vision)
		{
			...
			cvtColor(mat, img_hsv, COLOR_BGR2HSV);// cvtColor 함수를 이용하여 hsv 색공간으로 변환

			Mat yellow_mask, yellow_image,green_mask,green_image,red_mask,red_image;

			Scalar lower_yellow = Scalar(20, 20, 100);// hsv 이미지에서 바이너리 이미지로 생성
			...
			inRange(img_hsv, lower_yellow, upper_yellow, yellow_mask); // 범위내의 픽셀들은 흰색, 나머지 검은색
			bitwise_and(mat, mat, yellow_image, yellow_mask);  //바이너리 이미지를 마스크로 사용하여 원본이미지에서 범위값에 해당하는 영상부분을 획득
